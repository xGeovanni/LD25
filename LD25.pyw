...
class menuOption():
    def __init__(self, img, pos, clickedImg = None):
...
class playerInputBox():
    def __init__(self, font, colour, pos = None, maxlen = None):
        ...
        self.colour = colour
        self.maxlen = maxlen
        self.textImg = self.font.render(self.text, True, self.colour)
...
class GUIManager():
    ...
    def drawGUI(self, ship):
        texts = (self.font.render("Score:           " + str(ship.score),
                                True, (0, 0, 0)).convert_alpha(),
                 self.font.render("", True, (0, 0, 0)).convert(),
    ...
    def menu(self, cursor, sm, hsm):
        ...
        audioButtonText = self.largeFont.render("♫", True, (0, 255, 0)).convert_alpha()
        audioButtonClickedText = self.largeFont.render("♫", True, (255, 0, 0)).convert_alpha()
        audioButton = menuOption(audioButtonText,
                                 (WIDTH - audioButtonText.get_width(), 0),
                                  audioButtonClickedText)
        highScoreButtonText = self.font.render("High Scores", True, (0, 255, 0)).convert_alpha()
        ...
        txtGoat = []
        for line in strGoat:
            txtGoat.append(self.font.render(line, True, self.menuColour).convert_alpha())
    ...
    def lose(self): # Maybe defunct
        youLose = self.largeFont.render("You Lose!", True, self.menuColour)
        text = self.font.render("Press any key to return to the main menu", True, self.menuColour)
    ...
    def newHighScore(self):
        title = self.font.render("New High Score!", True, self.menuColour)
        text = self.font.render("Enter your name", True, self.menuColour)
...
class HPBar():
    def __init__(self, maxHP, anchor):
        ...
        self.x = self.anchor.x + self.anchor.img.get_width() / 2 - self.greenbar.get_width() / 2
        self.y = self.anchor.y - self.greenbar.get_height()
...
def play(gm, sm, cursor):
    ...
    units = [Person() for i in range(numunits)]
    spawn(Tank, units)
    ...
    unitClasses = (Person, Tank, Helicopter, Soldier)
